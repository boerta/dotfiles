...
if orig=$(hc get_attr monitors.${idx}.my_orig_rect 2> /dev/null ) ; then
    ...
    orig=( $(hc monitor_rect $idx) ) || exit 1
    left=( ${orig[0]} ${orig[1]} $((${orig[2]} / 2)) ${orig[3]} )
    leftedge=$(( ${left[0]} + ${left[2]} ))
    halfheight=$(( ${orig[3]} / 2 ))
    rightwidth=$((${orig[2]} - ${left[2]}))
    topright=( $leftedge ${orig[1]} $rightwidth ${halfheight} )
    bottomright=( $leftedge ${halfheight} ${rightwidth} ${halfheight} )
    leftrect=$(array2rect ${left[@]})
    toprightrect=$(array2rect ${topright[@]})
    bottomrightrect=$(array2rect ${bottomright[@]})
    hc chain \
        , lock                                                      \
        ...
        , sprintf ATTR "monitors.%s.my_orig_rect" monitors.count    \
            chain                                                   \
            . add_monitor "$toprightrect"                           \
            . new_attr string ATTR                                  \
            . set_attr ATTR "${orig[*]}"                            \
        , sprintf ATTR "monitors.%s.my_orig_rect" monitors.count    \
            chain                                                   \
            . add_monitor "$bottomrightrect"                        \
            . new_attr string ATTR                                  \
            . set_attr ATTR "${orig[*]}"                            \
        , unlock
fi
